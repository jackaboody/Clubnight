// presentation/controllers/scheduling_controller.dart
//
...
class SchedulingController extends AsyncNotifier<SchedulingState> {
  ...
  Future<SchedulingState> build() async {
    ...
    _matchSub = ref.watch(activeMatchesStreamProvider.stream).listen(
          _onMatchesChanged,
          onError: (e) => state = AsyncValue.data(
            SchedulingState(lastError: e.toString()),
          ),
        );

    ref.onDispose(() => _matchSub?.cancel());
  ...
  Future<void> _handleMatchesEnded(
    ...
    try {
      ...
      // Find which courts had the ended matches.
