    ...
    data: (playerList) => matches.when(
      data: (matchList) {
        ...
        final unavailable = playerList
            ...
            .length;
        final singlesCount =
            matchList.where((m) => m.type == MatchType.singles && m.status == MatchStatus.active).length;
        final doublesCount =
            matchList.where((m) => m.type == MatchType.doubles && m.status == MatchStatus.active).length;
...
class OrganizerStats {
  ...
  final double fairnessScore; // lower = fairer
