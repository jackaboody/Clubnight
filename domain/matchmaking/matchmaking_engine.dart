// domain/matchmaking/matchmaking_engine.dart
//
// Pure Dart — zero Firebase imports. Fully unit-testable.
//
// Scoring model (lower = better):
//   totalCost = waitCost + fairnessCost + skillCost
//
...
//
// The engine returns one MatchmakingResult per available court slot, each
// containing the optimal group for that slot. It is called by the
// CourtScheduler whenever a court becomes free or a new player joins.
...
class MatchmakingEngine {
      ...
      if (doublesEligible.length >= 4) {
        ...
        for (final group in doubles) {
          final cost = _score(
            ...
            config: config,
            recentPairings: recentPairings,
            // Slight bonus to reward doubles preference being honoured
            bonusMultiplier: 0.92,
          );
  ...
  double _waitCost(List<Player> group, DateTime now) {
    ...
    return -(totalWaitMinutes / group.length).clamp(0, 9999);
  }
